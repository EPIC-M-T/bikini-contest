...
function modelFromApproved(m,i){
  const cityState=[m.city,m.state].filter(Boolean).join(', ')||m.city||'Las Vegas, NV';
  return {
    id:m.id||m.modelId||m['Model ID']||`approved-${i+1}`,
    number:m.number||m.Number||i+1,
    name:m.name||m.Name||`Contestant ${i+1}`,
    city:cityState,
    status:'Selected',
    votes:Number(m.voteCount||m.votes||m['Vote Count']||0),
    photo:m.headshotUrl||m.image2Url||m.image3Url||m.photo||m['Headshot URL']||m['Image 2 URL']||m['Image 3 URL']||'assets/Prize-pool.webp',
    instagram:m.instagram||m['IG Handle']||''
  };
}
...
function addJudgingAndRegistrationInfo(){
  ...
  style.textContent=`
    ...
    .judging-note p{margin:0;color:#e3d6ea;font-size:1rem;line-height:1.55}.judging-note p+p{margin-top:10px}.judging-note .important{color:#fff8e9;font-weight:850}
  ...
  if(heroPanel&&!document.querySelector('.judging-note.hero-judging-note')){
    const note=document.createElement('div');note.className='judging-note hero-judging-note';
  ...
  const voteHead=document.querySelector('#vote .showcase-head h2');if(voteHead) voteHead.textContent='Meet The Girls. Support Fan Favorite.';
  const voteIntro=document.querySelector('#vote .showcase-head p');if(voteIntro){voteIntro.textContent='Approved contestants will be posted here as numbered model cards. The display supports up to 100 girls, searchable cards, status filters, and Fan Favorite audience support. The contestant with the most Fan Favorite votes receives a $2,500 Private Tailgate Cabana prize package. Overall contest winners are selected by judges, not by votes.';}
  const modelNote=document.querySelector('.model-note');if(modelNote){modelNote.textContent='Fan Favorite support is for audience engagement and the separate $2,500 Private Tailgate Cabana prize package only. Final placement, prize eligibility, and official results are determined by EPIC Models & Talent contest rules, judging criteria, and event operations.';}
  const applyLede=document.querySelector('#models .section-lede');if(applyLede){applyLede.textContent='Models interested in participating can apply online for the opportunity to be featured and promoted prior to the event. Day-of-event submissions will be accepted until 1:00 PM on August 15, 2026; however, spots are limited, so early registration is encouraged.';}
  const photosCheck=document.querySelector('#models .checklist .check:nth-child(2)');if(photosCheck){photosCheck.innerHTML='<b>02 · 3–5 Images</b>Show your face, physique, style, and range.';}
  const postingCheck=document.querySelector('#models .checklist .check:nth-child(4)');if(postingCheck){postingCheck.innerHTML='<b>04 · Selection + Posting</b>Selected girls will be notified by email and may be posted on the main page and social channels before the live, judge-selected competition.';}
}
...
function renderModels(){
  const grid=document.querySelector('[data-model-grid]'),counter=document.querySelector('[data-model-counter]'),posted=document.querySelector('[data-model-posted]'),search=document.querySelector('[data-model-search]'),filter=document.querySelector('[data-model-filter]'),empty=document.querySelector('[data-model-empty]');
  if(!grid)return;
  const stored=getVotes();
  let models=hydrateModels().map(m=>({...m,votes:m.votes+(stored[m.id]||0)}));
  function draw(){
    const q=(search?.value||'').toLowerCase().trim();
    const f=filter?.value||'all';
    const filtered=models.filter(m=>(!q||`${m.name} ${m.city} ${m.status}`.toLowerCase().includes(q))&&(f==='all'||String(m.status||'selected').toLowerCase().includes(f)));
    grid.innerHTML=filtered.map(m=>`<article class="model-card" style="--model-photo:url('${m.photo}')" data-card><span class="model-number">#${String(m.number).padStart(3,'0')}</span><div class="model-photo" aria-hidden="true"></div><div class="model-content"><span class="model-tag">${m.status||'Selected'}</span><h3 class="model-name">${m.name}</h3><p class="model-meta">${m.city||'Las Vegas, NV'} · Fan Favorite support open</p><div class="vote-row"><button class="vote-btn" type="button" data-vote="${m.id}">Support Fan Favorite</button><span class="vote-count" data-count="${m.id}">${m.votes}</span></div></div></article>`).join('');
    if(counter)counter.textContent=String(Math.min(models.length,EPIC_MODEL_LIMIT));
    if(posted)posted.textContent=String(models.filter(m=>String(m.status||'selected').toLowerCase().includes('selected')).length);
    if(empty)empty.hidden=filtered.length!==0;
    requestAnimationFrame(()=>document.querySelectorAll('[data-card]').forEach((c,i)=>setTimeout(()=>c.classList.add('is-visible'),i*55)));
  }
  grid.addEventListener('click',async e=>{
    const btn=e.target.closest('[data-vote]');if(!btn)return;
    const id=btn.dataset.vote;
    const voter=localStorage.getItem('epic-voter-key')||crypto?.randomUUID?.()||String(Date.now()+Math.random());
    localStorage.setItem('epic-voter-key',voter);
    try{await jsonp(EPIC_APPS_SCRIPT_URL+'?action=vote&modelId='+encodeURIComponent(id)+'&voterKey='+encodeURIComponent(voter)+'&source=main-homepage');}catch(err){console.warn('Vote backend did not update',err)}
    const votes=getVotes();votes[id]=(votes[id]||0)+1;setVotes(votes);
    models=models.map(m=>m.id===id?{...m,votes:m.votes+1}:m);
    const count=document.querySelector(`[data-count="${id}"]`);if(count)count.textContent=String(models.find(m=>m.id===id)?.votes||0);
    btn.textContent='Support Counted';setTimeout(()=>btn.textContent='Support Fan Favorite',1100)
  });
