...
function renderModels(){const grid=document.querySelector('[data-model-grid]'),counter=document.querySelector('[data-model-counter]'),posted=document.querySelector('[data-model-posted]'),search=document.querySelector('[data-model-search]'),filter=document.querySelector('[data-model-filter]'),empty=document.querySelector('[data-model-empty]');if(!grid)return;const stored=getVotes();let models=hydrateModels().map(m=>({...m,votes:m.votes+(stored[m.id]||0)}));async function voteFor(id){const voted=getVoted();if(voted[id])return;const voter=await getVoterKey();let backendDuplicate=false;try{const result=await jsonp(EPIC_APPS_SCRIPT_URL+'?action=vote&modelId='+encodeURIComponent(id)+'&voterKey='+encodeURIComponent(voter)+'&source=main-homepage');backendDuplicate=!!result?.duplicate;if(result&&typeof result.voteCount!=='undefined')models=models.map(m=>m.id===id?{...m,votes:Number(result.voteCount)||m.votes}:m)}catch(err){console.warn('Vote backend did not update',err)}if(!backendDuplicate){const votes=getVotes();votes[id]=(votes[id]||0)+1;setVotes(votes);models=models.map(m=>m.id===id?{...m,votes:m.votes+1}:m)}voted[id]=true;setVoted(voted);draw();const modal=ensureModal();const current=models.find(m=>m.id===id);if(modal.classList.contains('is-open')&&current)openModelModal(current,voteFor)}function draw(){const q=(search?.value||'').toLowerCase().trim();const f=filter?.value||'all';const voted=getVoted();const filtered=models.filter(m=>(!q||`${m.name} ${m.city} ${m.status} ${m.instagram} ${m.song}`.toLowerCase().includes(q))&&(f==='all'||String(m.status||'selected').toLowerCase().includes(f)));grid.innerHTML=filtered.map(m=>`<article class="model-card" style="--model-photo:url('${m.photo}')" data-card data-id="${m.id}"><span class="model-number">#${String(m.number).padStart(3,'0')}</span><div class="model-photo" aria-hidden="true"></div><div class="model-content"><span class="model-tag">${m.status||'Selected'}</span><h3 class="model-name">${escapeHtml(m.name)}</h3>${m.song?`<p class="model-song"><b>Walk-Out Song</b><br>${escapeHtml(m.song)}</p>`:''}<button class="model-detail-btn" type="button" data-details="${m.id}">View Profile + Photos</button><div class="vote-row"><button class="vote-btn" type="button" data-vote="${m.id}" ${voted[m.id]?'disabled':''}>${voted[m.id]?'Already Supported':'Support Fan Favorite'}</button><span class="vote-count" data-count="${m.id}">${Number(m.votes||0)}</span></div></div></article>`).join('');if(counter)counter.textContent=String(Math.min(models.length,EPIC_MODEL_LIMIT));if(posted)posted.textContent=String(models.filter(m=>String(m.status||'selected').toLowerCase().includes('selected')).length);if(empty)empty.hidden=filtered.length!==0;renderTopFive(models);requestAnimationFrame(()=>document.querySelectorAll('[data-card]').forEach((c,i)=>setTimeout(()=>c.classList.add('is-visible'),i*55)))}grid.addEventListener('click',e=>{const detail=e.target.closest('[data-details]');if(detail){const m=models.find(x=>x.id===detail.dataset.details);if(m)openModelModal(m,voteFor);return}const btn=e.target.closest('[data-vote]');if(btn&&!btn.disabled){voteFor(btn.dataset.vote);return}const card=e.target.closest('[data-card]');if(card&&!e.target.closest('button')){const m=models.find(x=>x.id===card.dataset.id);if(m)openModelModal(m,voteFor)}});search?.addEventListener('input',draw);filter?.addEventListener('change',draw);draw()}
