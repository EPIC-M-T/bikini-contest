...
function cleanFileName(name) {
  return String(name || 'upload').replace(/[^a-zA-Z0-9._-]+/g, '-').replace(/^-+|-+$/g, '') || 'upload';
}

async function packFile(file) {
  if (!file) return null;
  const dataUrl = await readFileAsDataURL(file);
  const type = mimeFromDataURL(dataUrl) || file.type || 'application/octet-stream';
  return { name: cleanFileName(file.name), type, dataUrl };
}
...
form?.addEventListener('submit', async event => {
  ...
  try {
    ...
    await postStage({
      ...
      shoeSize: document.querySelector('#shoeSize')?.value || '',
      naturalHairColor: document.querySelector('#hairColor')?.value || '',
      naturalEyeColor: document.querySelector('#eyeColor')?.value || '',
      favoriteMovie: document.querySelector('#favoriteMovie')?.value || '',
    ...
    showToast('Uploading reveal images...');
